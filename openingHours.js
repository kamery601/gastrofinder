function toMeterMinutes(hour = 0, minute = 0) {
...
function isWithinPeriod(dayIndex, minutes, openDay, openMin, closeDay, closeMin) {
  if (openDay === closeDay && openMin === closeMin) {
    return true;
  }
  if (openDay === closeDay) {
...
function isOpenAt(place, hour, minute, options = {}) {
  ...
  const minutes = toMeterMinutes(hour, minute);
  const periods = resolveOpeningPeriods(place);
  if (periods) {
    for (const period of periods) {
      if (!period.open || !period.close) continue;
      const openDay = period.open.day;
      const openMin = toMeterMinutes(period.open.hour, period.open.minute);
      const closeDay = period.close.day;
      const closeMin = toMeterMinutes(period.close.hour, period.close.minute);
...
module.exports = { isOpenAt, getDayIndex };
